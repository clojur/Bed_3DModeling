...
CDbHandlerInterface::CDbHandlerInterface(CViewControlData* pOSG)
{
	m_pOSG = pOSG;
...
void CDbHandlerInterface::finish()
{
	...
	m_pOSG->m_pSelector->setSelectFilter(flitter);//zmz


	CWnd* pWnd = m_pOSG->GetCurrentCWnd();
	if (pWnd && pWnd->GetSafeHwnd() )
		pWnd->SendMessage(WM_SETCURSOR, 0, 0);
}
void CDbHandlerInterface::SetConformPoint(osg::Vec3d confirmPT)
{
	lastConfirmPointBeValid = TRUE;
	lastConfirmPoint = confirmPT; 
}

void CDbHandlerInterface::clearCloneObject()
{
	//清除临时对象；
	std::map<osg::Node* , CDbObjInterface*>::iterator it2 = selectetSetMap.begin();
...
double CDbHandlerInterface::GetX(const osgGA::GUIEventAdapter& ea,osgViewer::View* viewer)
{
	double x =   ea.getX();
	if(viewer)
	{
...
double CDbHandlerInterface::GetY(const osgGA::GUIEventAdapter& ea,osgViewer::View* viewer)
{
	double y =  ea.getY();
	if(viewer)
	{
...
void CopyObjectAndAdd(CViewControlData* m_pOSG, CDbObjInterface* pCreateData)
{
	CString strPrix;
	strPrix.Format("%s_", pCreateData->GetTitle());


	pCreateData->SetID(CDbOSGDataCenter::GetID());
