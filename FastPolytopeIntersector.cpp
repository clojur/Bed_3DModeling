...
namespace FastPolytopeIntersectorUtils
{
    ...
    class FastPolytopeIntersection
	{
    public:
		enum { MaxNumIntesections = FastPolytopeIntersector::Intersection::MaxNumIntesectionPoints };
		FastPolytopeIntersection(unsigned int index, const CandList_t& cands, const osg::Plane &referencePlane) :
		...
		bool operator<(const  FastPolytopeIntersection& rhs) const { return _distance < rhs._distance; }
	...
	typedef std::vector< FastPolytopeIntersection> Intersections;

	class  FastPolytopePrimitiveIntersector
	{
    public:
        typedef osg::Polytope::PlaneList PlaneList;

        /// a line defined by the intersection of two planes
		struct  FastPlanesLine
        {
			FastPlanesLine(PlaneMask m, Vec3_type p, Vec3_type d) :
            mask(m), pos(p), dir(d) {}
            PlaneMask mask;
            Vec3_type pos;
            Vec3_type dir;
        };
		typedef std::vector< FastPlanesLine> LinesList;

		FastPolytopePrimitiveIntersector() :
            _index(0),
             _limitOneIntersection( false ),
			 _dimensionMask(FastPolytopeIntersector::AllDims),
             _plane_mask(0x0),
            _candidates(20) {}

            void addIntersection(unsigned int index, const CandList_t& cands) 
			{
				intersections.push_back(FastPolytopeIntersection(index, cands, _referencePlane));
			}
        ...
        unsigned int checkCandidatePoints(PlaneMask inside_mask)
        {
            ...
            for(PlaneList::const_iterator it=_planes.begin();  it!=_planes.end() && numCands>0; ++it, selector_mask <<= 1)
            {
        ...
        {
            ++_index;
			if ((_dimensionMask & FastPolytopeIntersector::DimZero) == 0) return;
        ...
        {
            ++_index;
			if ((_dimensionMask & FastPolytopeIntersector::DimOne) == 0) return;
        ...
        {
            ++_index;
			if ((_dimensionMask & FastPolytopeIntersector::DimTwo) == 0) return;
        ...
        {
            if ((_dimensionMask & FastPolytopeIntersector::DimTwo) == 0)
            {
                ++_index;
                return;
            }
        ...
        LinesList& getPolytopeLines()
        {
            ...
            for (PlaneList::const_iterator jt=it+1; jt!=_planes.end(); ++jt, sub_selector_mask <<= 1 ) {
                ...
                _lines.push_back(FastPlanesLine(selector_mask|sub_selector_mask, linePoint, lineDirection));
            }
            }
            return _lines;
        }
...
FastPolytopeIntersector::FastPolytopeIntersector(const osg::Polytope& polytope) :
    _parent(0),
    _polytope(polytope),
...
FastPolytopeIntersector::FastPolytopeIntersector(CoordinateFrame cf, const osg::Polytope& polytope) :
    Intersector(cf),
    _parent(0),
    _polytope(polytope),
    _dimensionMask( AllDims ),
...
FastPolytopeIntersector::FastPolytopeIntersector(CoordinateFrame cf, double xMin, double yMin, double xMax, double yMax) :
    Intersector(cf),
    _parent(0),
    _dimensionMask( AllDims ),
	bReserve(false)
...
Intersector*  FastPolytopeIntersector::clone(osgUtil::IntersectionVisitor& iv)
{
    if (_coordinateFrame==MODEL && iv.getModelMatrix()==0)
    {
		osg::ref_ptr<FastPolytopeIntersector> pi = new FastPolytopeIntersector(_polytope);
        pi->_parent = this;
	...
	osg::ref_ptr<FastPolytopeIntersector> pi = new FastPolytopeIntersector(transformedPolytope);
    pi->_parent = this;
	pi->bReserve = bReserve;
...
bool  FastPolytopeIntersector::enter(const osg::Node& node)
{
...
void  FastPolytopeIntersector::leave()
{
    // do nothing.
}


void  FastPolytopeIntersector::intersect(osgUtil::IntersectionVisitor& iv, osg::Drawable* drawable)
{
    if (reachedLimit()) return;
	if (!drawable) return;

	if (drawable->getNodeMask() == 0x0)
		return;

	const osg::BoundingBox&  drawAbleBounding = drawable->getBoundingBox();
	if (!drawAbleBounding.valid())
		return;
	if (bReserve)
	{
	...
	{
		if (!_polytope.containsAllOf(drawable->getBoundingBox())) return;
	}

	osg::TemplatePrimitiveFunctor< FastPolytopeIntersectorUtils::FastPolytopePrimitiveIntersector> func;
	...
	FastPolytopeIntersectorUtils::Intersections::const_iterator it = func.intersections.begin();  //只插入一个intersection即可
	const FastPolytopeIntersectorUtils::FastPolytopeIntersection& intersection = *it;


	//for (FastPolytopeIntersectorUtils::Intersections::const_iterator it = func.intersections.begin();
 ...
 //   {
	//	const  FastPolytopeIntersectorUtils::FastPolytopeIntersection& intersection = *it;
		...
		FastPolytopeIntersectorUtils::Vec3_type center;
        for (unsigned int i=0; i<intersection._numPoints; ++i)
        {
            center += intersection._points[i];
        }
		center /= FastPolytopeIntersectorUtils::value_type(intersection._numPoints);
...
void  FastPolytopeIntersector::reset()
{
    Intersector::reset();
